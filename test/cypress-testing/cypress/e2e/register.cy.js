...
describe("Register Page Testing", () => {
    ...
    afterEach(() => {
        // Delete newest user in database after each successful register

        cy.url().then((url) => {
            if (url == Cypress.config().baseUrl + registerPage.redirectUrl) {
                // register successful
                cy.request({
                    method: "GET",
                    url: Cypress.config().backendUrl + "/user",
                }).as("userList");

                cy.get("@userList").then((response) => {
                    const userList = response.body;

                    const userDelete = userList.reduce(function (
                        prev,
                        current
                    ) {
